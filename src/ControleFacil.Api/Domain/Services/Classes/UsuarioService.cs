...
namespace ControleFacil.Api.Domain.Services.Classes
{
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<UsuarioLoginResponseContract> Autenticar(UsuarioLoginRequestContract usuarioLoginRequest)
        {
            ...
            {
                throw new AuthenticationException("Usuário e/ou senha inválida.");
            }
        ...
        {
            _ = await Obter(idUsuario) ?? throw new NotFoundException("Usuário não encontrado para atualização.");
            var usuario = _mapper.Map<Usuario>(entidade);
            usuario.Id = id;
            usuario.Senha = GerarHashSenha(usuario.Senha);
        ...
        {
            var usuario = await _usuarioRepository.Obter();
